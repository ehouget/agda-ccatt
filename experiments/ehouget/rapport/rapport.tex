...
\subsection{Catégories} Une catégorie est la donnée d'une collection $\mathcal{C}_0$ d'objets, d'une collection $\mathcal{C}_1$ de morphimes (ou flèches) telles que : 
\begin{itemize}
...
\subsection{Catégories cartésiennes} Une catégorie $\mathcal{C}$ est dite cartésienne, si elle est munit d'un objet terminal $\mathbbm 1$ et si pour toute paire d'objets $A$ et $B$, il existe un produit $A \times B$ dans $\mathcal C$.
...
\subsection{Types} Les objets $\mathcal{C}_0  = \left\{x, y, \dots\right\}$ défini un ensemble de variables de type. Une \textbf{flèche} est un couple de variables de types, notée $x \rightarrow y$, avec $x$ la \textbf{source} et $y$ la \textbf{cible}. Ainsi tout morphisme $f$ de $\mathcal{C}_1$ est typé en notant $f : s(f) \rightarrow t(f)$.
...
\subsection{Termes} Un terme typé est un morphisme $f$ typé par une flèche $A$ et construit dans un contexte $\Gamma$. On défini inductivement le séquent $\Gamma \vdash f : x \rightarrow y$ par les règles suivantes :
\begin{center}
	\begin{prooftree}
		\hypo[]{f : A \in \Gamma}
...
On introduit un systeme d'écriture composé de 2 règles engendrant les termes normalisés. Ce système de réécriture doit apparaitre comme un intermédiaire entre le système très souple qui défini les termes, et le système de réécriture linéaire qui définira les schémas composables.
\begin{center}
	\begin{prooftree}
...
Comme nous l'avons fait dans la partie II, on introduit un systeme d'écriture engendrant des termes normalisés. Ces termes normalisés ont pour but d'être des représentants canoniques pour les classes d'équivalence de la relation $\sim$.
...
\bibitem{b2} Dominique Boucher, ``La Theorie des catégories en informatique :
