...
-- presence in a cross product
data _►_ {n : ℕ} : Ty n → Ty n → Set where
  var : {k : Fin n} → X k ► X k
  term : {A : Ty n} → 𝟙 ► A
  reorder : {A B : Ty n} → A × B ► B × A
  assoc : {A B C D : Ty n} → A ► (B × C) × D → A ► B × (C × D)
  merge : {A B C D : Ty n} → A ► B → C ► D → A × C ► B × D
  left : {A B C : Ty n} → A ► B → A ► B × C
  right : {A B C : Ty n} → A ► C → A ► B × C
